...
Authors: David Mutchler, Vibha Alangar, Matt Boutell, Dave Fisher,
         ...
         their colleagues, and Valeria Paiz.
...
def problem2b(rect, n, delta, win):
    ...
    rulx = rect.get_upper_left_corner().x
    ruly = rect.get_upper_left_corner().y
    rlrx = rect.get_lower_right_corner().x
    rlry = rect.get_lower_right_corner().y

    for k in range(n):
        rectangle = rg.Rectangle(rg.Point((rulx - k * delta), (ruly - k * delta)), rg.Point((rlrx + k * delta), (rlry + k * delta)))
